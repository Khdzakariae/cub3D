...
{
    if (!map || !map->map || x < 0 || x >= map->width * map->height 
        || y < 0 || y >= map->height * map->title_size)
        return 1;

    int mapX = (int)(x / map->title_size);
    int mapY = (int)(y / map->title_size);
...
int wall_hit(float x, float y, t_data *data)
{
    ...
    if (x < 0 || x >= data->game.map.width * TILE_SIZE || y < 0 || y >= data->game.map.height * TILE_SIZE)
        return 0; 
...
float get_v_inter(t_data *data, float angle)
{
    ...
    v_y = data->game.player.y + (data->game.player.x) * tan(angle);
...
void init_player(t_data *data){
    ...
    data->game.player.moveSpeed = 5;
    data->game.player.rotationSpeed = 2 * (M_PI / 180);
}
